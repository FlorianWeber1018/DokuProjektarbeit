...
Ist einer dieser Wechselschalter in Mittelstellung, wird die Bahn nicht versorgt und das entsprechende Auto steht, unabhängig des gewählten Modus.\\
		Über die Wechseltaster 0 beziehungsweise Wechseltaster 2 lässt sich der Modus der zugehörigen Bahn auswählen. Dies erfolgt indem einer der Taster oben bzw unten betätigt wird.  Der aktuell ausgewählte Modus wird durch die LEDs über bzw unter den Tastern signalisiert. Das LED oben signalisiert dabei den Manuellen Modus, indem das Auto durch den Handregler  gesteuert wird und ein LED unten, den Modus indem das Auto selbstständig in fährt.\\
...
Global existiert noch ein weiterer Taster um die Visualisierung (Rundenzeitmessung) zurückzusetzen. Dieser ist in Abblidung \ref{img:hmi} mit \glqq Reset Visualisierung \grqq beschriftet und befindet sich zwischen den zwei Tiefsetzstellern oben links.
	\section{Arduino}
...
In der ISR muss nun weiter, zwischen den zwei Möglichkeiten, unterschieden werden:
	...
	\subsection{Regler}\label{subsec:control}
		\begin{figure}[ht]
			...
			\caption{Spannungsregelung Bahn}
			\label{fig:Ucontrol}
		\end{figure}
		\begin{figure}[ht]
			\centering
			\includegraphics[width=\textwidth]{rec/Regler.pdf}
			\caption{Reglung Bahngeschwindigkeit}
			\label{fig:Tcontrol}
		\end{figure}
