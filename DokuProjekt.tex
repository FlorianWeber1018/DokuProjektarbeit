...
Global existiert noch ein weiterer Taster um die Visualisierung (Rundenzeitmessung) zurückzusetzen. Dieser ist in Abblidung \ref{img:hid} mit \glqq Reset Visualisierung\grqq{} beschriftet und befindet sich zwischen den zwei Tiefsetzstellern oben rechts.
	\section{Arduino}
...
Der Programmablauf ist interruptgesteuert und folgt damit dem Modell in Abbildung \ref{InterruptgesteuerterProgrammablauf}.
			...
			In der Mainloop des Programms werden zeitunkritische Aufgaben erledigt, wie die LEDs des HMI zu aktualisieren.
...
Zusammen bilden die zwei Regelkreise im automatisierten Modus eine Kaskadenregelung.
		\subsubsection{Bahnspannung}\label{subsubsec:Ucontrol}
			...
			Jedoch braucht ein Regelkreis mindestens einen freien Integrator, um stationäre Genauigkeit bei einer sprungartigen Sollwertänderung, aufzuweisen.
...
\emph{i\_control\_railspeed\_a}  (siehe Quellcode)  aufgerufen und darin die  Regeldifferenz \glqq e\grqq{} gebildet. Analog zu dem Regelkreis in Abschnitt \ref{subsubsec:Ucontrol}, wird auch hier die Regeldifferenz gebildet und mit dem Faktor K\textsubscript{Geschwindigkeit} skaliert. Als Regler kommt ein I-Regler, wie in Abschnitt \ref{subsubsec:Ucontrol}, zum Einsatz. Die so erzeugte Stellgröße U\textsubscript{Soll} wird nun auf das Stellglied (innerer Regelkreis Bahnspannung, Abschnitt \ref{subsubsec:Ucontrol}, Abbildung \ref{fig:Ucontrol}) gegeben und es ergibt sich durch eine neue Bahnspannung auch eine neue Zeit für den gemessenen Streckenabschnitt. In der nächsten Runde wiederholt sich der Kreis.\\
Um Fehler, die durch kurzzeitige externe Störungen verursacht werden, abzufangen ist die maximale Änderungsrate von U\textsubscript{Soll} nach oben hin begrenzt.
...
\newpage
Durch einen Wechsel des Modus aus dem Modus Auto-Run (siehe Abschnitt \ref{subsec:bedAuto}) in einen anderen Modus bleibt der aktuelle Wert des Integrators erhalten, sodass das Auto beim nächsten Wechsel in den Modus Auto-Run, vorausgesetzt die zu Beginn erwähnten Modellparameter haben sich nicht zu sehr geändert, bereits mit der richtigen Geschwindigkeit startet.
Ist dies nicht erwünscht ist der Regler, nach der Bedienungsanleitung in Kapitel \ref{cha:manual}, zurückzusetzen.
\FloatBarrier		
	...
	\subsection{Analog Digital Wandler}\label{subsec:ADC}
		...
		Die Steuerung des MUX erfolgt über einen Zustandsautomaten (siehe Abbildung \ref{img:ADCMUX}), der immer dann ein Schritt weiter springt, wenn dieser Interrupt eintritt. In jedem Schritt des Automaten, wird schematisch dasselbe durchgeführt:
		\begin{itemize}
...
\chapter{Bedienungsanleitung}\label{cha:manual}
%evt bild von visu
\section{Einschalten}
...
\section{Betrieb}
	...
	\end{itemize}
	Die Modi können unabhängig voneinander auf beiden Bahnen gewählt werden, sodass es zum Beispiel möglich ist, manuell gegen ein automatisiertes Fahrzeug anzutreten. Die Wahl eines dementsprechenden Modus (automatisiert oder manuell), wird durch eine LED über, beziehungsweise unter, des jeweiligen Tasters des gewählten Modus signalisiert.
Der Wechsel zwischen den Modi findet nach Abbildung \ref{img:Betriebsmodi} statt und ist in den jeweiligen Abschnitten genauer erklärt. Außerdem kann die Energieversorgung jeder Bahn mit dem entsprechenden Wechselschalter ausgewählt werden. Unabhängig des gewählten Modus, wird in der Visualisierung die Zeit der zwei Streckenabschnitte, sowie die Rundenzeit angezeigt.
...
Nach 30 Runden ist das Rennen vorbei und die Visualisierung wird statisch. Ein neues Rennen kann gestartet werden indem der Taster \glqq Reset Visualisierung \grqq{} (siehe Abbildung \ref{img:hid}) des HMI gedrückt wird.
Der Regler der Bahngeschwindigkeit der jeweiligen Bahn kann mit dem Wechseltaster 1 des HMI zurückgesetzt werden. 
...
\newpage
	\subsection{Automatisiertes Fahren}\label{subsec:bedAuto}
		...
		\subsubsection{Auto-Run}
			...
			braucht das Auto einige Runden, um die optimale Spannung für die Geschwindigkeit zu finden. Dies ist genauer in Abschnitt \ref{subsubsec:Tcontrol} beschrieben.
			Es ist darauf zu achten, dass wenn das Auto im Looping herunterfällt, es wieder vor der Startlinie eingesetzt wird. Ist dies nicht der Fall, 
...
\section{Ausschalten}
Um die Anlage ordnungsgemäß herunterzufahren, ist der Reset Taster der Visualisierung (Abbildung \ref{img:hid} - Reset Visualisierung) mindestens fünf Sekunden gedrückt zu halten.
...
Ein bestehendes Problem der Carrera Bahn ist, dass gelegentlich ein Auto im Looping herunterfällt, wenn dieses automatisch fährt. Die Ursache dessen wurde identifiziert (siehe Abschnitt \ref{cha:err}), kann allerdings nicht mit vertretbarem Aufwand gelöst werden.
